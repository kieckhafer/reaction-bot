...
function Botkit(configuration) {
    ...
    function Conversation(task, message) {
        ...
        this.trigger = function(event, data) {
            if (this.events[event]) {
                ...
                if(event != 'tick' && event != 'presence_change'){
    ...
    botkit.saveMessages = function(keywords, events){
        // TODO: Just add things that have hashtags
        var textKeys = keywords.join(',');
        botkit.allKeywords[textKeys] = '[na]';
    }
    ...
    botkit.hears = function(keywords, events, cb) {
        ...
        this.saveMessages(keywords, events);
